...
class IBM::Sip
    ...
    def on_update
        ...
        @domino = ::IBM::Domino.new({
            username: ENV['DOMINO_USERNAME'],
            password: ENV['DOMINO_PASSWORD'],
            auth_hash: ENV['DOMINO_HASH'],
            domain: ENV['DOMINO_DOMAIN'],
            timezone: 'Singapore'
        })
    ...
    def fetch_bookings
        ...
        # :setup => 0,
        # :breakdown => 0
        bookings = @domino.get_bookings(system.id)
        bookings.map!{|booking|
            {
                :Start => Time.at(booking[:start]).utc.iso8601[0..18],
                :End => Time.at(booking[:end]).utc.iso8601[0..18],
