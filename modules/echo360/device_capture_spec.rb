...
Orchestrator::Testing.mock_device 'Echo360::DeviceCapture' do
    ...
    # NOTE:: ignore warnings about private functions
    resp = Hash.from_xml(capture_status)
    exec(:process_status, resp)

    expect(status[:api_versions]).to eq('3.0')
    ...
    resp = Hash.from_xml(captures)
    expect(resp['captures']['capture'][0]['id']).to eq('0797b8dd-4c2d-415a-adf9-daf7f10e1759')
end
